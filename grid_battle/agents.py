...
def heuristic_turn(snapshot: BattleSnapshot) -> TurnAction:
    ...
    for enemy in snapshot.enemies:
        ...
        if result is not None:
            ...
            action2 = _find_second_attack(
                snapshot, player_position, enemy.position, blocking_positions, enemy_positions, player_profile
            )
            return TurnAction(
                move_direction=None,
    ...
    if attack_action is not None:
        action2 = _find_second_attack(
            snapshot, player_position, None, blocking_positions, enemy_positions, player_profile
        )
...
class RandomAgent:
    ...
    def act(self, snapshot: BattleSnapshot) -> TurnAction:
        ...
        for enemy in snapshot.enemies:
            ...
            if result is not None:
                attack_options.append(PhaseAction(result[0], result[1]))

        chosen_attack = self._rng.choice(attack_options)

        # randomly pick an item to activate (if any unused items are in the inventory)
